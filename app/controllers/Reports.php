...
class Reports extends MY_Controller
{
    ...
    function daily_sales($year = NULL, $month = NULL)
    {
        ...
        if(!empty($sales)) {
            foreach($sales as $sale){
                ...
                $html .= "<span class='text-warning'><div class='pull-left'>Taxa:</div> ". $sale->tax."</span><br>";
                $html .= "<span class=''><div class='pull-left'>Desc:</div> ". $sale->discount."</span><br>";
                $html .= "<span class='text-success'><div class='pull-left'>Vendas:</div> ". $sale->total."</span><br>";
                $html .= "<span style='border-top:1px solid #DDD;'><span class='pull-left'>Lucro:</span> ".$sale->grand_total."</span>";

                $html .= "<hr />";

                $payments = array("CC" => "Cartão", "Cheque" => "iFood", "cash" => "Dinheiro", "gift_card" => "Cartão Loja");

                foreach ($sale->payments as $key => $value) {

                    $payment_type = $this->site->getPaymentsByID($key);

                    $html .= "<span style='font-size:10px;display:block'><div class='pull-left'>".$payment_type->name.":</div> ".$value."</span>";
                }

                $daily_sale[$sale->date] = $html;;
            }
        } else {
            $daily_sale = array();
        }
    ...
    function get_sales()
    {
        ...
        $this->datatables
        ->select("id, date, customer_name, total, total_tax, total_discount, grand_total, paid, (total-paid) as balance")
        ->from('sales')
        ->unset_column('id');
