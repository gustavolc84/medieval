...
class Sales extends MY_Controller {
	...
	function get_sales()
	{
        ...
        $this->datatables->add_column("Actions", "<div class='text-center'><div class='btn-group'><a href='".site_url('sales/alert/$1')."' title='Pedido Pronto (Alertar)' class='tip btn btn-primary btn-xs' ><i class='fa fa-check'></i></a><a href='".site_url('sales/reopen/$1')."' title='Reabrir' class='tip btn btn-primary btn-xs' ><i class='fa fa-refresh'></i></a><a href='#' onClick=\"MyWindow=window.open('" . site_url('pos/view/$1/1') . "', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=yes,width=350,height=600'); return false;\" title='".lang("view_invoice")."' class='tip btn btn-primary btn-xs'><i class='fa fa-list'></i></a>  <a href='".site_url('sales/add_payment/$1')."' title='" . lang("add_payment") . "' class='tip btn btn-primary btn-xs' data-toggle='ajax'><i class='fa fa-money'></i></a> <a href='".site_url('sales/payments/$1')."' title='" . lang("view_payments") . "' class='tip btn btn-primary btn-xs' data-toggle='ajax'><i class='fa fa-briefcase'></i></a> <a href='" . site_url('pos/?edit=$1') . "' title='".lang("edit_invoice")."' class='tip btn btn-warning btn-xs'><i class='fa fa-edit'></i></a> <a href='" . site_url('sales/delete/$1') . "' onClick=\"return confirm('". lang('alert_x_sale') ."')\" title='".lang("delete_sale")."' class='tip btn btn-danger btn-xs'><i class='fa fa-trash-o'></i></a></div></div>", "id, status");

        //$this->datatables->unset_column('id');

        echo str_replace(array("done", "partial", "paid", "due"), array("Finalizado", "Não Pago", "Pago", "Não Pago"), $this->datatables->generate());
    ...
    function get_visor()
    {
        ...
        echo $this->sales_model->getVisorInvoice();
    ...
    function alert($id = NULL)
    {
        ...
        if ( $this->sales_model->alertInvoice($id) ) {
            $this->session->set_flashdata('message', "Senha chamada no visor");
        }
    ...
    function reopen($id = NULL)
    {

        if ( $this->sales_model->reopenInvoice($id) ) {
            $this->session->set_flashdata('message', "Pedido reaberto.");
        }
