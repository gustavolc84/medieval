...
class Pos extends MY_Controller {
	...
	{		
		...
		if ($this->form_validation->run() == true) {
			...
			for ($r = 0; $r < $i; $r++) 
			{
				...
				$item_flavor   = isset($_POST['flavor'][$r]) ? $_POST['flavor'][$r] : 0;
				$item_note     = isset($_POST['item_note'][$r]) ? $_POST['item_note'][$r] : '';
				$additional    = isset($_POST['adicionais'][$r]) ? $_POST['adicionais'][$r] : '';
				...
				if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
					...
					$products[] = array(
						...
						'flavor_id'       => $item_flavor,
						'item_note'       => $item_note,
						'additional'      => $additional,
						);

					$total += $item_net_price * $item_quantity;
					if($this->input->post('entrega') == 2){
						$total += 0.85;
					}
				}
			}
			...
			$data = array('date' => $date,
        		...
        		'customer_name' => (($this->input->post('cliente_loja') != '') ? $this->input->post('cliente_loja') : $customer),
				'customer_name' => (($this->input->post('cliente_loja') != '') ? $this->input->post('cliente_loja') : ''),
			...
			if (!$suspend && $paid) {
				...
				if($this->input->post('entrega') == '2'){
					$deliveries = [
						'customer_id'  => $customer_id,
						'change_money' => $payment['pos_balance'],
						'status'       => 1,
						'created'      => date('Y-m-d')
					];
				}
			...
			if($suspend) {
				...
				if($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
					...
					if(sizeof($deliveries) > 0){
						$deliveries['sale_id'] = $sale['sale_id'];
						$rs = $this->deliveries_model->addDelivery($deliveries);
					}
		...
		else
		{
			...
			$this->data['payment_types'] = $this->site->getAllPaymentTypes();
			$this->data['message'] = $this->session->flashdata('message');
			$this->data['suspended_sales'] = $this->site->getUserSuspenedSales();
			$this->data['flavors'] = $this->flavors_model->getAllCombo();
			$this->data['adicionais'] = $this->products_model->getAdicionais();
		...
		if($products) {
			if($this->Settings->bsty == 1) {
				...
				foreach($products as $product) {
					...
					if($count < 10) { $count = "0".($count /100) *100;  }
					if($category_id < 10) { $category_id = "0".($category_id /100) *100;  }
					$prods .= "<button type=\"button\" data-name=\"".$product->name."\" id=\"product-".$category_id.$count."\" type=\"button\" value='".$product->code."' class=\"btn btn-both btn-flat product\"><i>R$ ".$product->price."</i><span class=\"bg-img\"><img src=\"".base_url()."uploads/thumbs/".$product->image."\" alt=\"".$product->name."\" style=\"width: 100px; height: 100px;\"></span><span><span>".$product->name."</span></span></button>";
					$pro++;
				}
			}
		} else {
	...
	function view($sale_id = NULL, $noprint = NULL)
	{
		...
		$this->data['types_flavors'] = $this->flavors_model->getAll();
		$this->data['additionais'] = $this->pos_model->getAdditionaisItens($sale_id);
