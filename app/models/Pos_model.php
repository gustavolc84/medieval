...
class Pos_model extends CI_Model
{
    ...
    public function addSale($data, $items, $payment = array(), $did = NULL)
    {
        ...
        $paymentTax = ($percent*$paymentType->tax)+$paymentType->fix_tax;

        $data['grand_total'] = $total - $paymentTax;
        $data['total_tax'] = $paymentTax;

        if($this->db->insert('sales', $data)) {
            $sale_id = $this->db->insert_id();            
            foreach ($items as $item) {
                $item['sale_id'] = $sale_id;                
                $itens_adicionais = $item['additional'];
                unset($item['additional']);                
                if($this->db->insert('sale_items', $item)) {
                   $id_item = $this->db->insert_id();                    
                    if($itens_adicionais != ''){
                        $add = explode(',',$itens_adicionais);
                        foreach ($add as $value) {
                            $param = ['item_id' => $id_item , 'product_id' => $value];
                            $this->db->insert('item_product_additional', $param);
                        }
                    }
    ...
    public function getAdditionaisItens($id){
        $this->db->select('b.id , p.name')
                 ->from('item_product_additional a')
                 ->join('sale_items b','a.item_id = b.id')
                 ->join('sales c','b.sale_id = c.id')
                 ->join('products p','a.product_id = p.id')
                 ->where('c.id',$id);
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
