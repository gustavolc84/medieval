...
class Reports_model extends CI_Model
{
	...
	public function getDailySales($year, $month)
	{
		...
		$paymentsQuery = "SELECT DATE_FORMAT( date,  '%e' ) AS date, COALESCE(sum(amount), 0) as total, paid_by FROM (tec_payments)
		WHERE DATE_FORMAT( date,  '%Y-%m' ) =  '{$year}-{$month}'
		GROUP BY CONCAT(DATE_FORMAT( date,  '%e' ), '-', paid_by) ";
		$paymentsQ = $this->db->query($paymentsQuery, false);

		foreach (($paymentsQ->result()) as $row) {
			$payData[$row->date][$row->paid_by] = $row->total;
		}

		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				if(isset($payData[$row->date])){
					$row->payments = $payData[$row->date];
					$data[] = $row;	
				}				
			}
			return $data;
	...
	public function getMonthlySales($year)
	{
		...
		$paymentsQuery = "SELECT DATE_FORMAT( date,  '%c' ) AS date, COALESCE(sum(amount), 0) as total, paid_by FROM (tec_payments)
		WHERE DATE_FORMAT( date,  '%Y' ) =  '{$year}'
		GROUP BY CONCAT(DATE_FORMAT( date,  '%c' ), '-', paid_by) ";
		$paymentsQ = $this->db->query($paymentsQuery, false);
