...
function loadItems() {
    ...
    if (get('spositems')) {
        ...
        $.each(spositems, function () {
            ...
            if(typeof item.row.additionais === 'undefined'){
                item.row.additionais = [];
            }

            if(typeof item.row.flavor === 'undefined'){
                item.row.flavor = '';
            }

            if(typeof item.row.item_note === 'undefined'){
                item.row.item_note = '';
            }
...
$(document).ready(function(){
    /*
    jQuery('.checkbox-entrega label, .checkbox-entrega .icheckbox_square-green').on('click', function() {

        if($('input#entrega').val() == "0") {
            $('input#entrega').val(1);
        } else {
            $('input#entrega').val(0);
        }
    ...
    */
    ...
    //aqui
    jQuery('.checkbox-entrega label, .checkbox-entrega .iradio_square-green input[type="radio"]').on('click', function() {
        /*
            Entrega = 1
            Viagem  = 2 
        */
        //retorna o nome do radio selecionado [entregou , viagem]
        var texto = $(this).text();

        //retorna o valor do campo oculto que guarda o valor da variavel acima
        var entrega_viagem = $('input#entrega');

        // retorna o valor total do pagamento        
        var tt = parseFloat($('#twt').text());
        var balance     = parseFloat($('#balance').text());
        var balance_val = parseFloat($('#balance_val').val());
        
        if(texto == 'Entrega')
        {
            // se o usuario já tinha selecionado viagem e depois marcou entrega, retira os 0.85 centavos
            // acrescentado anteriormente
            if(entrega_viagem.val() == 2){
                tt -= 0.85;
                gtotal -= 0.85;
                ...
                $('#twt').text(tt.toFixed(2)); 
                $('#balance').text(balance.toFixed(2));
                $('#balance_val').val(balance_val.toFixed(2));
            }
            $('input#entrega').val(1);
        }
        else
        {
            //evita que seja somado mais vezes caso o usuario clique mais de uma vez na opcao viagem
            if(entrega_viagem.val() != 2){
                tt += 0.85;
                gtotal += 0.85;
                gtotal = gtotal.toFixed(2);
            ...
            entrega_viagem.val(2); 
        }

        // guarda o valor total do pagamento neste campo também
        $('#total-payable').text($('#twt').text());
    });
    ...
    $("#posTable").on("click", '.edit', function() {
        ...
        var item_category = row.attr('data-item-category');
        ...
        if(item_category == 5){        
            $('.div_flavors').css('display','block');
            $('.div_additional').css('display','none');
        }

        if(item_category == 4){        
            $('.div_flavors').css('display','none');
            $('.div_additional').css('display','block');            
        }        
    ...
    ============================= */
     $(document).on('click', '#editItem', function () {
        ...
        var flavor  = $('#nFlavor').val();
        var item_note = $('#nItemNote').val();
        var arrAdicionais = new Array();

        if(spositems[item_id].row.category_id == 4){
            $('input[name="nAdicionais[]"]').each(function(){                
                if($(this).is(':checked')){                    
                    arrAdicionais.push($(this).val());
                }            
            });
        }
        ...
        spositems[item_id].row.flavor    = flavor,
        spositems[item_id].row.item_note = item_note,
        spositems[item_id].row.additionais = arrAdicionais;
    ...
    $('#amount').change(function(e) {
        ...
        var vlr_adicional = 0.0;
        $('#total_paying').text(formatMoney(total_paying));

        if($('input#entrega').val() == 2){
            vlr_adicional = 0.85;
        }
        if(Settings.rounding != 0) {
            var vlr = formatMoney(total_paying - round_total);
            $('#balance').text(vlr);
            $('#balance_val').val(vlr);
            total_paid = total_paying;
            grand_total = round_total;
        } else {
            var vlr = formatMoney(total_paying - gtotal);
            $('#balance').text(vlr);
            $('#balance_val').val(vlr);
            total_paid = total_paying;
            grand_total = gtotal;
