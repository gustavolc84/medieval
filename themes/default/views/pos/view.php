...
if ($modal) {
    ...
    <head>
        <meta charset="utf-8">
        <title><?=  "Pedido " . lang("no") . " " . $inv->id; ?></title>
        <base href="<?= base_url() ?>"/>
...
<div id="wrapper">
    ...
    <div id="receipt-data">
        <div class="text-center">
                <?= $Settings->header; ?>
                <p>
                <div style="font-size:25px;"><?= lang('sale').' '.lang('no').': <b>'.substr($inv->id, -2); ?></b></div><br>
                <b><?= lang("customer").':</b> '. $inv->customer_name; ?> <br>

                <?php if($customer->address): ?>
                <b>Endereço:</b> <?= $customer->address; ?> <br>
                <?php endif; ?>
                
                <?php if($customer->phone && $inv->cliente_loja == ''): ?>
                <b>Telefone do Cliente:</b> <?= $customer->phone; ?> <br>
                <?php endif; ?>

                <b><?= lang("date").':</b> '.$this->tec->hrld($inv->date); ?>
            ...
            <table class="table table-condensed">
                ...
                foreach ($rows as $row) {
                    echo '<tr><td class="text-left"> <strong>' . product_name($row->product_name) . '</strong></td>';
                    ...
                    if ($row->flavor_id != 0) {
                        echo '<tr style = "background-color: #FFF; text-align: left;">
                                <td colspan = "4"> <span style = "margin-left: 10px;"><i class="fa fa-reply"></i> Sabor '. $types_flavors[$row->flavor_id] .'</span> </td>
                            </tr>';
                    }

                    if(sizeof($additionais) > 0){
                        foreach ($additionais as $key => $value) {
                            if($value->id == $row->id){
                                echo '<tr style = "background-color: #FFF; text-align: left;">
                                         <td colspan = "4"> <span style = "margin-left: 10px;"><i class="fa fa-reply"></i> Adicional: '. $value->name .'</span> </td>
                                      </tr>';
                            }
                        }
                    }
                    ...
                    if ($row->item_note != '') {
                        echo '<tr style = "background-color: #FFF; text-align: left;">
                                <td colspan = "4"> <span style = "margin-left: 10px;"><i class="fa fa-reply"></i> Obs: '. $row->item_note .'</span> </td>
                            </tr>';
                    }                                       
                }
                ...
                if($inv->entrega == '2'){
                    echo '<tr>
                            <th colspan = "2" class = "text-left">Taxa de Viagem</th>
                            <th colspan = "2" class="text-right">'. $this->tec->formatMoney(0.85) .'</th>
                        </tr>';
                }
                ?>
                </tbody>
                ...
                <tr>
                    <th colspan="2"><b><?= lang("total"); ?></b></th>
            ...
            if ($payments) {
                ...
                foreach ($payments as $payment) {
                    ...
                    echo '<tr>';
                    if ($payment->pos_paid) {
                        echo '<td><b>' . lang("paid_by") . ':</b> ' . $paymentType->name . '</td>';
                        echo '<td><b>' . lang("amount") . ':</b> ' . $this->tec->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid) . '</td>';
                        echo '<td><b>' . lang("change") . ':</b> ' . ($payment->pos_balance > 0 ? $this->tec->formatMoney($payment->pos_balance) : 0) . '</td>';
                ...
                echo ($inv->note) ? '<p class="text-center" style="border:2px solid #111;padding:10px 0"><b>Observação: </b>' . $this->tec->decode_html($inv->note) . '</p>' : '';
                
                $ex1 = new QRCodeGenerator('http://medievallanches.com.br/');
        ...
        <b><?= lang('sale').' '.lang('no').':</b> '.substr($inv->id, -2); ?><br>
        <b><?= lang("customer").':</b> '. $inv->customer_name; ?> <br>
            <?php 
                if($inv->entrega == "1") {
                    echo "<b>(Entrega)</b>";
                }elseif($inv->entrega == "2"){
                    echo "<b>(Viagem)</b>";
                }  
            ?>
        </h4>
        ...
        <?php 
            echo ($inv->note) ? '<p class="text-center" style="border:2px solid #111;padding:10px 0;font-size:20px;"><b>Observação: </b>' . $this->tec->decode_html($inv->note) . '</p>' : '';
         ?>

        <br><br>
        <div style="clear:both;"></div>
        <table style="font-size:20px;" class="table table-striped table-condensed">
            <thead>
                <tr>
